...
%                      Default is (0.999:0.00005:1.001).'.
...
DefV.ScaleVec        = (0.8:0.005:1.2).';  %(0.997:0.00005:1.003).';
...
for Iseg=1:1:Nseg,
    ...
    [~,Igrid] = min(plane_dist(MeanX,MeanY,SegmentsInfo(Iseg).MeanX,SegmentsInfo(Iseg).MeanY));
...
%         SpecWave = interp1(ShStCalibTemplatePix,CalibTemplateWave,SpecPix,InPar.Interp1Method);
        ...
        if (length(find(~isnan(SpecWave))) < 5)
                    % do nothing - bad solution
            SegmentsInfo(Iseg).MaxCorr        = NaN;
            SegmentsInfo(Iseg).Wave_LineResid = NaN;
            SegmentsInfo(Iseg).Wave_LineNum   = 0;
